...
export default function ProductDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <Link href="/products" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardBody>
            <h2 className="mb-4 text-base font-semibold text-gray-800">Điểm cơ hội</h2>
        ...
        <Card>
          <CardBody className="space-y-3">
            <h2 className="text-base font-semibold text-gray-800">Giá & tồn kho</h2>
      ...
      <Card>
        <CardBody>
          <h2 className="mb-4 text-base font-semibold text-gray-800">Biến thể</h2>
          {(!p.variants || p.variants.length === 0) ? (
            <p className="py-6 text-center text-sm text-gray-400">Chưa có biến thể</p>
          ) : (
                ...
                <tbody>
                  {p.variants.map((v: any) => (
                    <tr key={v.id} className="border-b border-gray-50 last:border-0">
                      <td className="py-2.5 text-gray-700">{v.variantSku}</td>
                      <td className="py-2.5 text-gray-600">{v.size ?? '—'}</td>
                      <td className="py-2.5 text-gray-600">{v.color ?? '—'}</td>
                      <td className="py-2.5 text-right font-medium text-gray-900">{v.stockQuantity}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {content && (
        <Card>
          <CardBody>
            <h2 className="mb-3 text-base font-semibold text-gray-800">Nội dung do Content AI tạo</h2>
            {content.error ? (
              ...
              <div className="space-y-3">
                {content.variations.map((v: any) => (
                  <div key={v.version} className="rounded-lg border border-gray-100 bg-gray-50/60 p-3 text-sm text-gray-700">
                    <div className="mb-1 text-xs font-medium text-gray-400">Phiên bản {v.version}</div>
                    {v.content}
                  </div>
                ))}
              </div>
